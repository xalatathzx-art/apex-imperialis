 ...
 *
 * This regenerates one slice per actor pack, <collection>.reused-items.mjs,
 * from two sources:
...
/**
 * Actor packs to fill, each with the item pack its NPCs copy from (if any), the
 * index collection its slices belong to, and a rank: where two packs translate
 * the same item differently, the higher-ranked name wins — the core book first.
 */
const BOOKS = {
  core: { actors: "navis-core-actors", items: "navis-core-items", collection: "impmal-core.actors", label: "Актёры (Основная книга)", rank: 4 },
...
const GENERATED = collection => `${collection}.reused-items.mjs`;
...
const actorsOf = {};
const itemSources = [
  ...Object.values(BOOKS).filter(b => b.items).map(b => [b.items, b.rank + 10]),
  ...EXTRA_ITEMS.map(pack => [pack, 5])
];
for (const [pack, rank] of itemSources) {
  const items = await read(pack);
  const tr = built(pack);
  for (const item of items.primary) {
    const k = key(item.type, item.name);
    offer(k, tr[item._id]?.name, rank);
    offerText(k, text(item), tr[item._id]?.description);
  }
}
for (const [book, { actors }] of Object.entries(BOOKS)) actorsOf[book] = await read(actors);

const embeddedOf = ({ primary, embedded }) => {
  const byId = new Map(Object.values(embedded).map(doc => [doc._id, doc]));
  return primary.map(actor => {
    const items = (actor.items ?? []).map(id => byId.get(id)).filter(Boolean);
    // Names an actor carries more than once — "Forbidden" for Lore and again for
    // Linguistics. A translation keyed by name lands on every copy, so these are
    // only ever addressed by _id, which Babele checks first.
    const seen = new Map();
    for (const item of items) seen.set(item.name, (seen.get(item.name) ?? 0) + 1);
    return { actor, items, repeated: new Set([...seen].filter(([, n]) => n > 1).map(([name]) => name)) };
  });
};

for (const [book, { rank, actors }] of Object.entries(BOOKS)) {
  const tr = built(actors);
  for (const { actor, items, repeated } of embeddedOf(actorsOf[book])) {
    for (const item of items) {
      const byId = tr[actor._id]?.items?.[item._id];
      const t = byId ?? tr[actor._id]?.items?.[item.name];
      const k = key(item.type, item.name);
      // Twins share an English name, so the Russian name is safe to learn from
      // either. Their texts differ, and a name-keyed description cannot be told
      // apart from its twin's — that pairing teaches nothing about this copy.
      offer(k, t?.name, rank);
      if (byId || !repeated.has(item.name)) offerText(k, text(item), t?.description);
    }
  }
}
...
async function authored(collection) {
  ...
  for (const file of fs.readdirSync(SRC)) {
    if (!file.startsWith(`${collection}.`) || file === GENERATED(collection)) continue;
...
for (const [book, { collection, label }] of Object.entries(BOOKS)) {
  const done = await authored(collection);
  const entries = {};
  let n = 0, d = 0;
  for (const { actor, items, repeated } of embeddedOf(actorsOf[book])) {
    const out = {};
    for (const item of items) {
      const slot = repeated.has(item.name) ? item._id : item.name;
      const marks = done.get(actor._id);
      const have = new Set([...(marks?.get(slot) ?? []), ...(repeated.has(item.name) ? [] : marks?.get(item._id) ?? [])]);
      const k = key(item.type, item.name);
      const entry = {};
      ...
      // An _id entry is all Babele reads for that copy, so it must carry the name
      // too — take the translated one if this copy's name is English.
      if (slot === item._id && !entry.name && english(item.name) && names.has(k)) entry.name = names.get(k).name;
      out[slot] = entry;
    }
  ...
  fs.writeFileSync(path.join(SRC, GENERATED(collection)), js);
  console.log(`${collection}: ${n} names, ${d} descriptions reused across ${Object.keys(entries).length} actors`);
}
