...
- `fitImplant(actor, item, { test = true })`:
  ...
  - when `test` is true — run `await actor.setupSkillTest("medicae", { appendTitle: ` — ${item.name}`, fields: { disadvantage: 1 } })`, then read the result. **One, not `true`:** impmal treats the field as a numeric counter and interpolates it straight into the roll breakdown, so a boolean prints "Помеха: true" to the table.
...
export function energyCapacity(actor) {
  ...
  for (const item of implantsOf(actor)) {
    ...
    // resolveEntries, NOT a raw walk of the groups. It is what decides which
    // entries apply: an OR group with no recorded choice contributes nothing,
    // and a chosen OR group contributes exactly one entry. Walking the raw
    // arrays counts an energy entry the player was offered but did not pick.
    for (const entry of resolveEntries(item.system.mechanics, item.system.chosenEffects ?? {})) {
      if (entry?.kind === "energy") added += resolveQualityValue(entry.value, item.system.quality);
    }
  }

  // The TOTAL is clamped, never each contribution: a negative per-level value
  // is a legitimate book effect (a worse implant at level 1).
