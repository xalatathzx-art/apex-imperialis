...
**Note for the implementer:** the trigger name `dialog` and the field names `args.fields.SL`, `args.fields.advantage`, `args.fields.disadvantage`, `args.skill` were read out of impmal's own `effectScripts` in `impmal.js` and out of `module/config/weapon-trait-effects.js`. If step 3 shows no effect, log `args` inside the script to confirm the shape before changing anything else.
