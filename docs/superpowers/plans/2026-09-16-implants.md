...
- **Module id** is `apex-imperialis`. The item type id is exactly `apex-imperialis.implant`.
...
- Produces:
  - `IMPLANT_TYPE` — the string `"apex-imperialis.implant"`
...
- Modify: `module/apex-imperialis.js` — import and call `registerImplantModel()` in the `init` hook, `registerImplantSheet()` in `ready`
...
3. Open the console; confirm no `apex-imperialis |` errors.
...
`styles/implants.css`, scoped under the sheet's own class so it cannot leak. Follow `styles/biomonitor.css` for the module's visual vocabulary. Add the file to `module.json`'s `styles` array alongside `styles/apex-skin.css`.
...
- `fitImplant(actor, item, { test = true })`:
  ...
  - on success — set `system.installed = true`, roll `1d10`, compute `adaptationDays(roll, actor.system.characteristics.tgh.bonus)`, store `flags.apex-imperialis.adaptation = { days, since: game.time.worldTime }`, and post a card naming the days.
...
- emits `"type": "apex-imperialis.implant"`, not `"augmetic"`
...
- `type` is `apex-imperialis.implant`
