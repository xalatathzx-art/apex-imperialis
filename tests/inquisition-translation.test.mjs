...
test("Inquisition Guide item translation covers every official document ID", () => {
  ...
  const translated = Object.keys(JSON.parse(fs.readFileSync(new URL("../compendium/impmal-inquisition.items.json", import.meta.url), "utf8")).entries).sort();
...
test("Inquisition Guide table translation covers every official document ID", () => {
  ...
  const translated = Object.keys(JSON.parse(fs.readFileSync(new URL("../compendium/impmal-inquisition.tables.json", import.meta.url), "utf8")).entries).sort();
...
test("referenced Inquisition table rows are named from the items translation", () => {
  const tables = JSON.parse(fs.readFileSync(new URL("../compendium/impmal-inquisition.tables.json", import.meta.url), "utf8")).entries;
...
test("every Inquisition Guide interface key has a Russian translation", () => {
  const english = flatten(JSON.parse(fs.readFileSync(new URL("../../impmal-inquisition/lang/en.json", import.meta.url), "utf8")));
  const missing = Object.keys(english).filter(key => !RU[key]);
